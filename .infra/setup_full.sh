...
# Install packages individually (safer approach)
log "Installing git..."
apt install -y git && success_log "git installed successfully" || 
error_log "Failed to install git"

log "Installing docker.io..."
apt install -y docker.io && success_log "docker.io installed 
successfully" || error_log "Failed to install docker.io"

log "Installing python3-pip..."
apt install -y python3-pip && success_log "python3-pip installed 
successfully" || error_log "Failed to install python3-pip"

log "Installing awscli..."
apt install -y awscli && success_log "awscli installed 
successfully" || error_log "Failed to install awscli"

log "Installing basic tools..."
apt install -y curl wget unzip nodejs openssh-server jq htop vim 
nano && success_log "Basic tools installed" || error_log "Some basic 
tools failed"
...
# Function to get all block devices with details
get_block_devices() {
    lsblk -J -o NAME,SIZE,MOUNTPOINT,TYPE | jq -r '.blockdevices[] | 
    "\(.name) \(.size) \(.mountpoint // "none") \(.type)"'
}
...
# Enhanced EBS volume detection with multiple strategies
EBS_DEVICE=""
EBS_SUCCESS=false
...
for attempt in {1..60}; do
    ...
    for nvme_device in /dev/nvme*n1; do
        if [ -e "$nvme_device" ]; then
            DEVICE_SIZE=$(lsblk -b -n -o SIZE "$nvme_device" 2>/dev/
            null | head -1)
            if [ ! -z "$DEVICE_SIZE" ]; then
                # Convert to GB (approximately)
                device_size_gb=$((DEVICE_SIZE / 1024 / 1024 / 1024))
                log "🔍 Found NVMe device $nvme_device with size: 
                $$device_size_gb GB"
                
                # Check if size is around our target (48-52GB range 
                for 50GB volume)
                if [ "$$device_size_gb" -ge 48 ] && [ 
                "$$device_size_gb" -le 52 ] && ! is_mounted 
                "$nvme_device"; then
                    # Double check this is not the root device
                    if ! lsblk "$nvme_device" | grep -q "/"; then
                        EBS_DEVICE="$nvme_device"
                        success_log "Found EBS volume: $EBS_DEVICE 
                        ($$device_size_gb GB)"
                        EBS_SUCCESS=true
                        break 2
                    fi
    ...
    if [ -z "$EBS_DEVICE" ]; then
        ...
        while IFS= read -r line; do
            ...
            if [ "$device_type" = "disk" ] && [ "$mount_point" = 
            "none" ]; then
                # Check if size contains our target
                if echo "$device_size" | grep -E "(4[89]|5[0-2])(\.|
                G|$)"; then
                    full_device="/dev/$device_name"
                    if [ -e "$full_device" ] && ! is_mounted 
                    "$full_device"; then
                        EBS_DEVICE="$full_device"
                        success_log "Found EBS volume via strategy 
                        2: $EBS_DEVICE ($device_size)"
                        EBS_SUCCESS=true
                        break 2
                    fi
                fi
    ...
    # Strategy 3: AWS CLI approach (if available)
    if [ -z "$EBS_DEVICE" ] && command -v aws >/dev/null 2>&1; then
        ...
        meta-data/instance-id 2>/dev/null)
        if [ ! -z "$INSTANCE_ID" ]; then
            aws ec2 describe-volumes \
                ...
                --output text 2>/dev/null | while read device; do
                    if [ "$device" != "None" ] && [ ! -z 
                    "$device" ]; then
                        # Convert CloudFormation device name to 
                        actual device
                        actual_device=$(echo "$device" | sed 's/xvd/
                        nvme/; s/f$/1n1/')
                        if [ -e "$actual_device" ] && ! is_mounted 
                        "$actual_device"; then
            ...
            if [ -f /tmp/ebs_device ]; then
                ...
                rm -f /tmp/ebs_device
                if [ ! -z "$EBS_DEVICE" ]; then
                    success_log "Found EBS volume via AWS CLI: 
                    $EBS_DEVICE"
                    EBS_SUCCESS=true
                    break
                fi
...
# --- EBS 볼륨 attach/format/mount 검증 ---
MOUNT_OK=false
if [ "$EBS_SUCCESS" = "true" ] && [ ! -z "$EBS_DEVICE" ]; then
    ...
    if mount "$EBS_DEVICE" /mnt/data; then
        ...
        if [ ! -z "$DEVICE_UUID" ]; then
            echo "UUID=$DEVICE_UUID /mnt/data ext4 defaults,nofail 0 
            2" >> /etc/fstab
            log "📝 Added to fstab with UUID: $DEVICE_UUID"
        else
            echo "$EBS_DEVICE /mnt/data ext4 defaults,nofail 0 2" 
            >> /etc/fstab
        ...
        if df -h | grep -q "/mnt/data"; then
            ...
            ')
            success_log "EBS volume mount verified - Available 
            space: $MOUNT_SIZE"

            # ------------------------------
            # Configure Docker data-root AFTER EBS is mounted
            # ------------------------------
            log "🐳 Relocating Docker data directory to /mnt/data/
            docker..."
            systemctl stop docker 2>/dev/null || true
            mkdir -p /mnt/data/docker
            echo '{"data-root":"/mnt/data/docker",
            "log-driver":"json-file","log-opts":{"max-size":"100m"}}
            ' > /etc/docker/daemon.json
            systemctl daemon-reload
            if systemctl enable docker && systemctl start docker; 
            then
                success_log "Docker reconfigured to use /mnt/data/
                docker"
...
if [ "$MOUNT_OK" = true ]; then
    ...
    # Verify requirements.txt from repository
    ...
    if [ -f "requirements.txt" ]; then
        ...
        echo "qiskit>=1.0.0" >> requirements.txt
    fi

    # Clean system before heavy installations
    ...
    log "Available disk space before Python packages: 
    $$AVAILABLE_SPACE_BEFORE KB"

    # Install core Python packages step by step
    log "🐍 Installing Python packages in stages..."

    # Stage 1: Core scientific computing
    log "Stage 1: Installing core packages..."
    pip3 install --no-cache-dir --upgrade pip
    pip3 install --no-cache-dir "numpy>=1.24.0" && success_log "numpy 
    installed" || error_log "numpy failed"

    # Check space after numpy
    ...
    pip3 install --no-cache-dir "jupyter>=1.0.0" "notebook>=7.0.0" && 
    success_log "Jupyter installed" || error_log "Jupyter failed"

    # Stage 3: Basic utilities
    log "Stage 3: Installing utilities..."
    pip3 install --no-cache-dir "tqdm>=4.65.0" && success_log "tqdm 
    installed" || error_log "tqdm failed"

    # Stage 4: Quantum computing packages (most critical)
    ...
    if pip3 install --no-cache-dir "qiskit>=1.0.0"; then
        ...
        pip3 install --no-cache-dir "qiskit==1.0.2" && success_log 
        "Qiskit 1.0.2 installed" || error_log "Qiskit installation 
        failed"
    fi
    ...
    # Start Jupyter on host (simpler approach)
    ...
    # Docker container setup with enhanced error handling
    ...
    log "🚀 Next: Test environment, then request GPU quota for g4dn.
    2xlarge"
