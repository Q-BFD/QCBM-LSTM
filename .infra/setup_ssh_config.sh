...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    if ssh -o ConnectTimeout=10 -o BatchMode=yes "$ALIAS_NAME" "echo '✅ EC2 SSH 연결 성공!'" 2>/dev/null; then
        ...
        if ssh -o ConnectTimeout=10 -o BatchMode=yes "${ALIAS_NAME}-container" "echo '✅ Container SSH 연결 성공!'" 2>/dev/null; then
            ...
            echo "🎉 모든 SSH 연결 테스트를 통과했습니다."
        else
            echo "⚠️  Container SSH 연결 실패 (컨테이너가 아직 준비 중일 수 있음)"
            echo "💡 잠시 후 다음 명령으로 다시 시도해보세요: ssh ${ALIAS_NAME}-container"
        fi
    else
        ...
        echo "   2. 보안 그룹에서 포트 22 (EC2)와 2222 (Container)가 열려 있는지 확인하세요."
